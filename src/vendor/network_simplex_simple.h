...
public:
  ...
  ProxyObject<T> operator[](const int id) { return ProxyObject<T>(this, id); }
...
{
public:
  ProxyObject(SparseValueVector<T>* v, int idx)
  {
    _v = v;
    _idx = idx;
  };
  ...
  public:
    ...
    bool findEnteringArc()
    {
      ...
      for (e = _next_arc; e != _search_arc_num; ++e) {
        ...
        if (--cnt == 0) {
          a = fabs(_pi[_source[_in_arc]]) > fabs(_pi[_target[_in_arc]]) ? fabs(_pi[_source[_in_arc]])
...
public:
  ...
  int divid(int x, int y) { return (x - x % y) / y; }
...
private:
  ...
  void updateTreeStructure()
  {
    ...
    while (stem != u_out) {
      ...
      // Update u and right
      u = _last_succ[stem] == _last_succ[par_stem] ? _rev_thread[par_stem] : _last_succ[stem];
      right = _thread[u];
    }
    ...
    if (join != old_rev_thread && v_in != old_rev_thread) {
      for (u = v_out; u != up_limit_out && _last_succ[u] == old_last_succ; u = _parent[u]) {
  ...
  {
    Cost sigma =
      _forward[u_in] ? _pi[v_in] - _pi[u_in] - _cost[_pred[u_in]] : _pi[v_in] - _pi[u_in] + _cost[_pred[u_in]];
    ...
    while (pivot.findEnteringArc()) {
      if (max_iter > 0 && ++iter_number >= max_iter && max_iter > 0) {
        ...
        sprintf(errMess,
                ...
                "in cycle even though the solution has been reached, to check if it's the case here have a look at the "
                "minimal reduced cost. If it is very close to machine precision, you might actually have the correct "
      ...
      if (iter_number % 1000 == 0 || iter_number % 1000 == 1) {
        ...
        double sumFlow = 0;
        double a;
        a = (fabs(_pi[_source[in_arc]]) >= fabs(_pi[_target[in_arc]])) ? fabs(_pi[_source[in_arc]])
                                                                       : fabs(_pi[_target[in_arc]]);

...
std::vector<std::function<PredictionResult()>> configure_tasks(
  ...
  bool copredictMode, const std::vector<bool>& usable, const std::string& rngState, const int seed, IO* io,
  bool keep_going(), void all_tasks_finished())
{
  ...
  LibraryPredictionSetSplitter splitter(explore, full, shuffle, crossfold, usable, rngState, seed);
...
std::vector<PredictionResult> run_tasks(const std::shared_ptr<ManifoldGenerator> generator, Options opts,
                                        ...
                                        const std::string& rngState, const int seed, IO* io, bool keep_going(),
